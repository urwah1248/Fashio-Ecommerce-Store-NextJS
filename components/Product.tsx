...
const Product = ({product, ...props}: Props) => {
  ...
  return (
    <div className='product-card border-black border-2 p-4 sm:w-[304px] w-3/4'>
        <Link href="#" className='text-black'>
          <div className="hover:scale-[105%] transition-[250ms]">
            <img  
            src={product.images[0]}
            alt='ujasdh'
            className='sm:w-72 w-full h-52 object-cover object-top rounded-md'
            />
            <h4 className='text-center max-w-full mt-2'>{product.title}</h4>
          </div>
        </Link>
        <p className='text-center font-inter text-gray-500'>Rs.{Comma(productPrice)}</p>
        <p className='text-center font-inter text-gray-500'>Rating: {product.rating}</p>
